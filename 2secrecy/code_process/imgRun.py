...
class Img_run:
    ...
    def __init__(self):
        ...
        if not os.path.exists("img_out"):
            os.makedirs("img_out")
            print("img_out文件夹已经创建成功！")
        else:
            print("img_out文件夹已存在！")
        #   创建log文件夹
        if not os.path.exists("./code_process/log"):
            os.makedirs("./code_process/log")
            print("./code_process/log文件夹已经创建成功！")
        else:
            print("./code_process/log文件夹已存在！")
    ...
    def process_log(self, path_read, path_write, combina, radius):
        ...
        if self.Print_Save == True:
            ...
            # 把输出重定向文件
            print_log = open("./code_process/log/%s.log" % time_now, 'w')
